 ...
 *   3. Three-tab form: Receipt | Expense | Message
 ...
 *
 * TYPES:   receipt | expense | message
 * STATUSES: pending | self_reported | verified | approved | rejected
 *
 * FILE UPLOAD:
 *   Bucket: community-docs  (private)
 *   Path:   {submission_id}/{timestamp}-{filename}
 *   After insert the signed URL is not needed in the record — we store
 *   the storage path as document_url so admin can generate a signed URL.
 */
...
/**
 * Called by PortalAuth once the rep has a valid session AND their
 * email matches an approved submission.
 */
    ...
    <div class="portal">
      ...
      <div class="portal__section" id="portal-form-section">
        ...
        <p class="portal__section-desc">
          Log receipts (donations received), expenses (funds applied), or messages
...
function _renderForm(tab) {
  ...
  if (tab === 'receipt')  area.innerHTML = _receiptFormHtml();
...
/**
 * Receipt form
 * Represents: Stage 3 of the transparency pipeline
 * "We received a donation from a donor."
 * Fields: amount, description, notes, file upload
 */
function _receiptFormHtml() {
  return `
    <form class="portal__form" id="portal-record-form" novalidate>
      <div class="portal__form-row">
        ...
        <div class="portal__field">
          <label for="pf-description">Source / Donor Description</label>
      ...
      <div class="portal__field">
        ...
        <input type="file" id="pf-file" name="file"
      ...
      <button type="submit" class="portal__submit">Submit Receipt</button>
    </form>
  `;
}
...
/**
 * Expense form
 * Represents: Stage 4 of the transparency pipeline
...
/**
 * Message form
 * A free-text update from the community rep to admins / the public record.
 * No financial fields — just description + notes.
 */
...
async function _handleSubmit(tab, form) {
  ...
  try {
    ...
    // ─ Validation ────────────────────────────────────────────
    if ((tab === 'receipt' || tab === 'expense') && (!amount || amount <= 0)) {
    ...
    if (file) {
      ...
      const storagePath = `${_submissionId}/${timestamp}-${safeName}`;
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        type:          tab,              // receipt | expense | message
        status:        'pending',        // admin promotes to verified/approved
        amount:        amount,
        currency:      'USD',
...
async function _loadHistory() {
  ...
  const { data, error } = await supabase
    .from('community_financials')
    .select('id, type, status, amount, currency, description, submitted_at')
...
function _buildHistoryItem(row) {
  const typeLabel = { receipt: 'Receipt', expense: 'Expense', message: 'Message' }[row.type] || row.type;
